...
/** Real-time is opt-in: without a configured Reverb key the admin relies on on-demand loads. */
 ...
 * `broadcastAs` name (e.g. `registration.completed`). A no-op when realtime
 * isn't configured, so callers keep their on-demand loads as the baseline.
 */
