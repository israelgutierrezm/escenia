...
- **Opt-in del directorio (privacidad)**: `networking_opt_in` en `attendees` (por defecto OFF); el directorio solo lista a quienes optaron y no se puede solicitar conexión/reunión a quien no optó (422 `networking_unavailable`). Endpoint `GET/PUT /attend/networking/preferences` + banner de activación en la pestaña «Personas».
- **Notificaciones en vivo**: `NetworkingNotification` (ShouldBroadcast en canal PRIVADO `attendee.{ulid}`) emitido a la parte correspondiente en solicitud/respuesta de conexión y en propuesta/respuesta/cancelación de reunión. La auth de broadcasting del asistente (`/attend/broadcasting/auth`) firma además su canal privado propio (rechaza el ajeno, 403); el frontend (`usePrivateChannel` en LiveView) muestra un toast por aviso. Verificado E2E en vivo (auth 200; propuesta → `NetworkingNotification` encolado → worker → Reverb).
- **Diferido**: matchmaking sugerido. Ver TD-033.
...
- **Checkout del comprador (UI pública)**: vista `CheckoutView` en `/e/:eventId/entradas` (sin token, como el registro) — lista las entradas a la venta (`api.tickets`), selector de cantidad (respeta `remaining`), subtotal en vivo, datos del comprador y **cupón**; al confirmar crea el pedido con el descuento (subtotal/descuento/total) y muestra la confirmación (redirect al proveedor si el intent trae `redirect_url`, si no «pendiente de pago») + acceso al evento con el token del comprador. Verificado E2E en vivo (2×entrada + VERANO25 → total 75 MXN). El **paso de pago real con tarjeta sigue gated** al SDK del gateway.
- **Diferido (TD-022)**: reembolso parcial/por ítem; cupones a nivel tenant/globales; endurecer el tope de usos bajo concurrencia; integrar el paso de pago real (Stripe.js/redirect) en la página de checkout.
