...
- `RenderCertificatePdfAction` arma un VO `CertificateData` (sin filtrar el modelo al renderer). Endpoint **público** `GET /certificates/{code}/pdf` (resuelto sin scope por el código opaco = credencial compartible, throttled, `Content-Disposition: attachment`), render **on-demand**. Frontend: botón «Descargar certificado (PDF)» en la vista de verificación del asistente.
- **Verificado**: 2 tests (PDF `%PDF` por código; 404 desconocido) + Pint/PHPStan L6; E2E en vivo (descarga real del servidor → 200 `application/pdf`).
- **Diferido (TD-031)**: pre-generar en Job + guardar en object storage para volumen; plantilla con logo/tipografía del BrandKit + QR al verificador.
